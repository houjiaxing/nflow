...
history.insert = function (aInfo) {
    ...
    let arrays = [uid, sub_project, new_state, last_state, time, mask];
    ...
    if (is_block != undefined) {
        ...
        sql += `(uid,sub_project,new_state,last_state,time,mask,is_block,block_message) values (?,?,?,?,?,?,?,?)`;

    } else {
        sql += `(uid,sub_project,new_state,last_state,time,mask) values (?,?,?,?,?,?)`;
    }
